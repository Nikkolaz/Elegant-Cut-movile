  ...
  Future<List<Map<String, dynamic>>> getBarbers() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // El backend podría devolver { data: [...] } o simplemente [...]
        final List<dynamic> data = decoded is Map && decoded.containsKey('data') ? decoded['data'] : decoded;
        
        return data.map((b) {
          // Adaptar el modelo del backend al que usa la UI (ej. 'name', 'specialty', 'rating')
          return {
            'id': b['id_usuario'],
            ...
            'price': 'Desde \$25', // Hardcoded until pricing is dynamic
            'expression': 0, // Carita aleatoria o fija
